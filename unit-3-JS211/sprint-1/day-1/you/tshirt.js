const product1 = ({image,category,price},id) => {
 ...
 const display = () => {
     ...
     let data2 = JSON.parse(localStorage.getItem("t-shirt")) || [];
 
     let container = document.getElementById("container");
     container.innerHTML = null;
 
     data2.forEach((el,index) => {
         container.append(product2(el,index))
     });
 ...
 let remove = (id) => {
    ...
    let data2 = JSON.parse(localStorage.getItem("t-shirts")) || [];
 
    //step:-2 splice the data from LS
    data2.splice(id,1)
 
    //step:-3 update the data
    localStorage.setItem("t-shirts",JSON.stringify(data));
